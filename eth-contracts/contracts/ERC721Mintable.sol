...
contract Ownable {
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(newOwner != address(0), 'The new owner address is not a real address');
...
contract Pausable is Ownable{
    ...
    function setPausable(bool newStatus) public onlyOwner {
        if (_paused != newStatus)
            _paused = newStatus;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != currentOwner, 'The address to transfert to is the current owner');

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == currentOwner || isApprovedForAll(currentOwner, to), 'The address to transfert to is not approved by the owner');
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_tokenOwner[tokenId] == from, 'The address to transfert from must be the owner of the token');

        // TODO: require token is being transfered to valid address
        require(to != address(0), 'given address to transfert to is not valid');
